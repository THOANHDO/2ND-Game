...
const Home: React.FC<HomeProps> = ({ featuredProducts, onAddToCart }) => {
  ...
  // Initialize data synchronously for instant render
  const [heroSlides, setHeroSlides] = useState<HeroSlide[]>(() => StorageService.getHeroSlides());
  const [siteConfig, setSiteConfig] = useState<SiteConfig>(() => StorageService.getSiteConfig());
  const [products, setProducts] = useState<Product[]>(() => StorageService.getProducts());

  useEffect(() => {
    // Keep listener for dynamic updates
    const loadData = () => {
